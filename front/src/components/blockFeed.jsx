...
function BlockFeed() {
  ...
  const [filter, setFilter] = useState('all'); // State for time filter
  ...
  useEffect(() => {
    ...
    const endpoint = showCreated ? `createdBlock/${uid}` : `receivedBlock/${uid}`

    const fetchThreads = async () => {
      try {
        const res = await axios.get(`http://localhost:4000/blockFeedThreads/${endpoint}`) // Adjusted endpoint
        if (res.data.message) {
    ...
    fetchThreads()
  }, [showCreated]) // Dependency on showCreated to refetch when toggled
  ...
  return (
    <div>
      ...
      <div className='button-container'>
        ...
        <button className="create-thread-button" onClick={() => navigate('/createThread', { state: { defaultReceiverType: 'block' } })}>
          Create Thread
        </button>
      </div>
      <div className='button-container'> 
      ...
      <div className="thread-list">
        {filteredThreads.map(thread => (
          <div key={thread.tid} className="thread-card" onClick={() => navigate(`/threads/${thread.tid}`)}>
            <h2>{thread.subject} - <small>{showCreated ? `Sent to Block` : `Posted by ${thread.username}`}</small></h2>
